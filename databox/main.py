...
async def call_external_api(pipeline: str = None, opportunities: str = None, source: str = None, name: str = None, status: str = None, monetaryValue: int = None):
    try:
        headers = {
        ...
        async with httpx.AsyncClient() as client:
            ...
            pipeline_id = next((p["id"] for p in pipelines if p["name"] == pipeline), None)
            ...
            if opportunities is not None:
                filtered_opportunities = [op for op in opportunities_data if op["pipelineStageId"] == opportunities]
            elif source is not None:
                filtered_opportunities = [op for op in opportunities_data if op["source"] == source]
            elif name is not None:
                filtered_opportunities = [op for op in opportunities_data if op["name"] == name]
            elif status is not None:
                filtered_opportunities = [op for op in opportunities_data if op["status"] == status]
            elif monetaryValue is not None:
                filtered_opportunities = [op for op in opportunities_data if op["monetaryValue"] == monetaryValue]
            else:
